...
V2.2 工程化升级：GitHub 双仓库托管 + CI/CD 自动化构建流水线落地
基于 V2.1 混合开发架构基础，完成项目版本规范化管理与企业级 DevOps 工程化能力升级，实现代码云端托管、多仓库同步、提交自动构建、镜像自动打包，彻底从 “本地手动部署” 升级为 “云端自动化交付” 标准模式，补齐应届生最稀缺的工程化、自动化、协作化能力。
实现功能
完成项目 阿里云 Codeup + GitHub 双仓库托管，实现代码安全备份 + 面试开源展示
接入 GitHub Actions CI/CD 自动化流水线，代码提交自动触发构建流程
实现自动化能力：自动拉取代码 → 自动 JDK 环境配置 → 自动 Maven 打包 → 自动 Docker 镜像构建
完成工程化规范：版本迭代记录、分支管理、自动化验证、云端构建校验
核心解决问题
问题 12：本地手动构建繁琐、易出错，缺乏云端验证机制
问题描述
每次更新代码必须手动执行 mvn clean package、docker build 等命令；
无法保证他人拉取代码后可正常构建，缺乏统一、标准、可复现的构建流程；
本地环境差异（JDK 版本、Maven 版本、操作系统）会导致 “我本地能跑，别人跑不了”。
根本原因
缺乏自动化构建验证机制，所有构建流程依赖人工操作；
无统一标准化构建环境，项目可交付性、可移植性无法保证。
解决方案
接入 GitHub Actions 企业级 CI/CD 流水线：
代码提交至 GitHub 自动触发构建，无需人工干预
使用 GitHub 官方标准 Ubuntu 环境，保证构建环境统一、干净、可复现
自动完成 Maven 编译打包、Docker 镜像构建，任何构建失败立即暴露
实现代码即交付的工程化规范，任何时间、任何人员拉取代码均可稳定构建
问题 13：项目代码无云端托管，无版本回溯、无面试展示入口
问题描述
项目仅存在本地电脑，无备份、无版本管理可视化、无远程仓库；
面试无法提供代码地址，无法展示项目真实完整性、迭代记录、工程规范。
解决方案
采用双仓库托管策略（企业级标准方案）：
阿里云 Codeup：私有仓库，用于代码安全存储、本地同步、版本管理
GitHub：公开仓库，用于面试展示、项目开源、CI/CD 流水线运行
一条命令同步双仓库：git push origin master + git push github master
完整保留 commit 记录、版本迭代、文件修改历史，面试官可直接追溯项目成长轨迹
问题 14：多模块微服务项目，CI/CD 自动化构建路径、模块打包错误
问题描述
GitHub 自动化构建时，Maven 打包失败、Docker 构建失败，提示找不到 jar 包、找不到 Dockerfile；
因项目采用多模块结构（gateway、user-service 等），默认根目录构建无法定位子模块服务。
根本原因
CI/CD 流水线未指定模块构建，在根目录执行打包无法生成 gateway 服务 jar；
Docker 构建未进入对应模块目录，导致镜像构建失败。
企业级解决方案
CI/CD 配置中指定模块打包：mvn -pl gateway clean package
构建 Docker 镜像前进入对应服务目录：cd gateway && docker build -t mine-gateway:latest .
流水线与项目结构完全匹配，实现多模块微服务自动化构建兼容